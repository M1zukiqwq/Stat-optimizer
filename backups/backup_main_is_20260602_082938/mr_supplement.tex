...
\subsection{Full-stack TPC-H safety: reproduces fresh, no bad tail}
\label{sec:mr-tpch}
...
once at full repetition and assign it to every method sharing that plan; on these
six queries the six statistics states collapse to seven distinct plans, so this
both removes redundant timing and surfaces directly that calibrated and fresh
statistics induce identical plans.

The accuracy and runtime story is stable across seeds. Stale statistics are badly
wrong on the drifted date columns (scan Q-error $\approx 8.2$), and calibration
repairs them to fresh quality (v3 and the Router $\approx 1.05$, fresh
$\approx 1.05$). Calibrated statistics reproduce the fresh-statistics runtime
\emph{exactly} (geometric-mean ratio $1.00$ vs fresh on every seed), because they
induce the same plans, and run $\approx 10\%$ \emph{faster} than stale
($0.89$--$0.91\times$): here the stale mis-estimate picks a worse plan, so leaving
statistics stale is the slower option. Crucially for deployment safety, there are
\emph{zero} queries that regress more than $15\%$ relative to stale under any
calibrated method on any seed---no bad tail. The single query that changes plan
stale$\to$fresh runs $1.75\times$ slower under stale than under the calibrated or
fresh statistics. Maintaining these statistics requires one forward pass plus the
