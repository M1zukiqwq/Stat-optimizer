...
def build_single_column_rows(args) -> List[dict]:
    ...
    full = _smoke_summary(args.soft_full_dir)        # Soft (full)
    rec = _smoke_summary(args.soft_recent8_dir)      # Soft (recent8)
    ...
    return [
        {"name": "OASIS", **sc(cal.get("oasis_full"))},
        {"name": "Soft (full)", **sc(full.get("oasis_soft_projection"))},
        {"name": "Soft (recent $k{=}8$)", **sc(rec.get("oasis_soft_projection"))},
        {"name": "Soft", **sc(cal.get("oasis_soft_projection"))},
        {"name": "Hybrid (no soft)", **sc(cal.get("residual_hybrid"))},
        {"name": "\\textbf{Router}", **sc(cal.get("calibrated_hybrid"))},
    ]
...
def build_optimizer_map(args) -> Dict[str, dict]:
    ...
    return {
        "OASIS": op(cal.get("oasis_projected")),
        "Soft (full)": op(full.get("oasis_soft_projection")),
        "Soft (recent $k{=}8$)": op(rec.get("oasis_soft_projection")),
        "Soft": op(cal.get("oasis_soft_projection")),
        "Hybrid (no soft)": op(cal.get("hybrid")),
        "\\textbf{Router}": op(cal.get("calibrated_hybrid")),
    }
...
def emit_safety_table(args, out_dir: Path) -> None:
    ...
    lines = [
        ...
        "\\begin{table}[h]",
        "\\centering",
        "\\caption{Future selectivity Q-error by OOD drift family and DML trace. Soft variants",
        "differ only in feedback weighting: full-window, recent-window, and conflict-gated ($\\tau{=}0.03$). Soft (full)",
        "fails on the strong batch-load and range-shift families and on append/returns",
        "traces; Soft (recent8) and Soft remove those failures.}",
        "\\label{tab:stage2-app-safety}",
        "\\setlength{\\tabcolsep}{5pt}",
        "\\adjustbox{max width=.98\\textwidth}{%",
        "\\begin{tabular}{lrrrr}",
        "\\toprule",
        "Family / trace & OASIS & Soft (recent8) & Soft & Soft (full) \\\\",
        "\\midrule",
...
def emit_pg_table(args, out_dir: Path) -> None:
    ...
    def row(label, method):
        ...
        dev = fpct(r.get('new_plan_deviation_frac'), '{:.2f}\\%')
    ...
    lines = [
        ...
        "shift $\\times$ two seeds, 504 queries). Router is identical to",
        ...
        "\\midrule",
        row("OASIS", "oasis_projected"),
        row("Soft", "oasis_soft_projection"),
        row("Hybrid (no soft)", "hybrid"),
        row("Router", "calibrated_hybrid"),
        "\\bottomrule",
        "\\end{tabular}",
        "}",
        "\\end{table}",
