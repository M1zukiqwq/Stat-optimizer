"""train_mlp_model.py — train the attention-pooled MLP for KLL quantile correction."""
...
def _load_training_set(
    ...
    for path in files:
        ...
        tensor_record = tensorize_sample(
            ...
            use_time_decay=False,   # timestamp feature removed
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train attention-pooled MLP for KLL quantile correction")
    parser.add_argument(
        "--train-glob", default="training_data/*.json",
        help="Glob pattern for JSON training samples",
    )
    parser.add_argument(
        "--output-model", default="artifacts/kll_mlp_model.json",
...
def main() -> None:
    ...
    print(f"Training MLP ({args.hidden_dims}) on {len(features)} samples …")
