...
SQL_FILE="/home/tianqc/presto-optimizer/presto-cdf-simulation/job_experiment/drift/drift_heavy.sql"

echo "======================================================================"
echo "重度漂移注入 - 15 Rounds"
echo "======================================================================"
...
for round in $(seq 1 15); do
    ...
    $PRESTO_CLI --server $SERVER --catalog iceberg --schema imdb --file $SQL_FILE 2>&1 | grep -E "(INSERT|DELETE|rows)" | head -15
    echo ""
done
