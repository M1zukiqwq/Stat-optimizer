...
roughly one percentage point at every budget, and this holds across two training seeds on the full
...
the feedback-constraint system. The exact linear object is the vector of cell masses on
the partition induced by the histogram grid and the distinct feedback endpoints; a
feedback predicate constrains an exact sum of those cell masses, and after projection the
marginal is pinned only along the span of those constraints, leaving the orthogonal
complement (the nullspace) free for the prior to fill. A histogram on the fixed
$B{=}10$ grid exposes only $B{-}1{=}9$ interior boundaries, so Table~\ref{tab:mr_rank}
reports, per budget $K$, an \emph{operational} rank on these exposed boundaries---a
deployable proxy for the exact cell-level rank---and the residual free degrees of freedom
over the held-out cases.
...
per-budget choices follow the rank mechanism above, and the regime matters. On the
optimizer-decision proxy, where future predicates are spread so that residual nullspace
persists even at $K{=}16$, it keeps selecting the projected learned completion at the
dense budget (about $51\%$ projected OASIS versus $33\%$ ISOMER). On the \emph{dense
planner} configurations, where the queried windows pin the active constraints, it instead
selects ISOMER on all twelve configurations---the regime where the marginal is pinned and
the candidates coincide.
...
\subsection{TPC-H runtime check: a narrow six-query sanity test}
\label{sec:mr-tpch}
