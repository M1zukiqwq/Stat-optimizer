...
class PostgreSQLExperimentRunner:
    ...
    def run_query(self, query_id: str, sql: str, timeout: int = 300) -> Dict:
        ...
        try:
            ...
            # 保存前2000字符用于调试
            if isinstance(explain_output, str):
                result['plan_raw'] = explain_output
            else:
                result['plan_raw'] = json.dumps(explain_output, ensure_ascii=False)
...
def main():
    parser = argparse.ArgumentParser(description='PostgreSQL TPC-DS Benchmark Experiment')
