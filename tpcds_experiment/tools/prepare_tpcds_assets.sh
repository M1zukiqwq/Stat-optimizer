...
Options:
  ...
  --fixed-query-dir PATH Output directory for PostgreSQL-fixed SQL files
...
FIXER="$SCRIPT_DIR/fix_tpcds_queries_for_pg.py"

if [[ -z "$DATA_DIR" ]]; then
  DATA_DIR="$KIT_DIR/generated_data_sf${SCALE}"
fi
if [[ -z "$RAW_QUERY_DIR" ]]; then
  RAW_QUERY_DIR="$EXPERIMENT_ROOT/generated_queries_pg_raw_sf${SCALE}"
fi
if [[ -z "$FIXED_QUERY_DIR" ]]; then
  FIXED_QUERY_DIR="$EXPERIMENT_ROOT/generated_queries_pg_sf${SCALE}"
fi
